...
sap.ui.define(['./EYButton', './TabBar', './TabBarItem'],
	function (EYButton, TabBar, TabBarItem) {
		"use strict";
		...
		PageRenderer.render = function (oRm, oPage) {
			...
			oRm.write("<div");
			oRm.addClass("eyDPageCustomContainer");
			oRm.writeClasses();
			...
			/*	oRm.write('<div ');
				oRm.addClass("eyDPageFooter");
				oRm.writeClasses();
				oRm.write("><div class='eyDFooterInnerContent'>");
				oRm.renderControl(oTabBar1);
		...
		/*	var oSelect2 = new sap.m.Select({
			...
			var oTabBar1 = new TabBar({
				alignHeader: 'left',
				alignTabContent: 'middle',
				expand: true,
				type: 'black',
				customHeader: [new sap.m.Text({
					text: 'Sort By:'
				}).addStyleClass('sapUiSmallMarginBeginEnd'), oSelect2],
				items: [
					new TabBarItem({
						text: "Variations",
...
if(oPage.getCustomFooter()){
		oRm.write('<div ');
		if(oPage.getShowHeader()){
			oRm.addClass("eyDPageCustomFooterWrapper");
		}
				oRm.writeClasses();
	oRm.write('> ');
			$.each(oPage.getCustomFooter(), function (key, value) {
		...
		PageRenderer._isLightHeader = function (oPage) {
			...
			while (oParent) {
				...
				if (((sParentName === "sap.m.Popover" || sParentName === "sap.m.Dialog") && sChildName === "sap.m.NavContainer") || ((sParentName ===
							"sap.ui.comp.smartvariants.SmartVariantManagement" || sParentName === "sap.ui.comp.smartvariants.SmartVariantManagementUi2" ||
							sParentName === "sap.ui.comp.variants.VariantManagement" || sParentName === "sap.ui.fl.variants.VariantManagement") && sChildName ===
						"sap.m.ResponsivePopover")) {
					return true;
				}

				if (oParent && ["sap.m.SplitApp", "sap.m.SplitContainer"].indexOf(sParentName) > -1 && sChildName === "sap.m.NavContainer" &&
					/\-Master$/.test(oChild.getId())) {
					return true;
				}

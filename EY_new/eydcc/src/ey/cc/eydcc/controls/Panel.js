...
sap.ui.define(['jquery.sap.global', './../library', 'sap/ui/core/Control','sap/ui/core/IconPool',
	'sap/ui/Device',
	'./PanelRenderer','sap/m/library'],
	function (jQuery, library, Control,IconPool, Device, PanelRenderer,mLibrary) {
		"use strict";
// shortcut for sap.m.PanelAccessibleRole
	var PanelAccessibleRole = mLibrary.PanelAccessibleRole;
	...
	var BackgroundDesign = mLibrary.BackgroundDesign;
		 ...
		 * @constructor
		 * @public
		 * @alias ey.cc.eydcc.controls.Panel
		 * @ui5-metamodel This control/element also will be described in the UI5 (legacy) designtime metamodel
		 */
	 
var Panel = Control.extend("ey.cc.eydcc.controls.Panel", /** @lends sap.m.Panel.prototype */ { metadata: {
		library: "ey.cc.eydcc",
		properties: {
	...
	Panel.prototype.setHeight = function (sHeight) {
		...
		if (oDomRef) {
			...
			if (parseFloat(sHeight) != 0) {
				oDomRef.querySelector(".eyDPanelContent").style.height = sHeight;
			}
	...
	Panel.prototype.onAfterRendering = function () {
		...
		/*	oPanelContent && $icon.attr("aria-controls", oPanelContent.id);*/
				...
				$icon.attr("aria-controls", oPanelContent.id);
			...
			if (this.getExpanded()) {
				...
				$this.children(".eyDPanelExpandablePart").hide();
				//ARIA
	...
	Panel.prototype._createIcon = function () {
		var that = this,
			sCollapsedIconURI = IconPool.getIconURI("sys-add"),
		...
		}).addStyleClass("eyDPanelExpandableIcon");
	};
	...
	Panel.prototype._setContentHeight = function () {
		...
		oPanelContent = thisDomRef && thisDomRef.querySelector(".eyDPanelContent");
	...
	Panel.prototype._toggleExpandCollapse = function () {
		var src=this.oIconCollapsed.getSrc();
		if(src==="sap-icon://sys-add"){
		this.oIconCollapsed.setSrc("sap-icon://sys-minus");	
		}
		else{
			this.oIconCollapsed.setSrc("sap-icon://sys-add");		
		}
		var oOptions = {};
		...
		this.$().children(".eyDPanelExpandablePart").slideToggle(oOptions);
	};
	...
	Panel.prototype._toggleCssClasses = function () {
		...
		$this.children(".eyDPanelWrappingDiv").toggleClass("eyDPanelWrappingDivExpanded");
		$this.children(".eyDPanelWrappingDivTb").toggleClass("eyDPanelWrappingDivTbExpanded");
/*		$this.find(".eyDPanelExpandableIcon").first().toggleClass("eyDPanelExpandableIconExpanded");
*/	};

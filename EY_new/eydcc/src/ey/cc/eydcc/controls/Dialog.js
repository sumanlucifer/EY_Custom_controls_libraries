...
sap.ui.define(['jquery.sap.global', './../library', 'sap/ui/core/Control','sap/m/Bar',
	...
	function (jQueryGlobal, library, Control,Bar,
	InstanceManager,
	...
	Configuration) {
		...
		/**
		 * Constructor for a new Dialog Control.
		 *
		 ...
		 * @constructor
		 * @public
		 * @alias ey.cc.eydcc.controls.Dialog
 ...
 // shortcut for sap.ui.core.OpenState
		...
		// shortcut for sap.m.DialogType
		var DialogRoleType = mlibrary.DialogRoleType;
		...
		* @ui5-metamodel This control/element also will be described in the UI5 (legacy) designtime metamodel
		*/
		var Dialog = Control.extend("ey.cc.eydcc.controls.Dialog", /** @lends sap.m.Dialog.prototype */ {
			metadata: {
				interfaces: [
		...
		Dialog._bPaddingByDefault = (sap.ui.getCore().getConfiguration().getCompatibilityVersion("eyDDialogWithPadding").compareTo("1.16") < 0);
		...
		Dialog.prototype.init = function () {
			...
			if (Device.os.ios && Device.system.phone && !this._bMessageType) {
				this.oPopup.setModal(true, "eyDDialogTransparentBlk");
			} else {
				this.oPopup.setModal(true, "eyDDialogBlockLayerInit");
			}
		...
		Dialog.prototype.close = function () {
			...
			this.$().removeClass('eyDDialogDisableTransition');
		...
		Dialog.prototype._setDimensions = function () {
			...
			if ((bStretch && !bMessageType) || (bStretchOnPhone)) {
				this.$().addClass('eyDDialogStretched');
			}
...
*/		
			if (Device.chrome && this.getStretch()) {
...
//forcing repaint
		...
		Dialog.prototype._initBlockLayerAnimation = function () {
			this.oPopup._hideBlockLayer = function () {
				...
				$blockLayer.removeClass("eyDDialogTransparentBlk");
		...
		Dialog.prototype._getFirstFocusableContentSubHeader = function () {
			var $subHeader = this.$().find('.eyDDialogSubHeader');
			var sResult;
		...
		//Manage "eyDDialogWithSubHeader" class depending on the visibility of the subHeader
		...
		Dialog.prototype.setSubHeader = function (oControl) {
			...
			if (oControl) {
				oControl.setVisible = function (isVisible) {
					this.$().toggleClass('eyDDialogWithSubHeader', isVisible);
		...
		Dialog.prototype.setBeginButton = function (oButton) {
			...
			if (oButton && oButton.isA("sap.m.Button") && sTheme.startsWith("sap_fiori_")) {
				...
				oButton.addStyleClass("eyDDialogBeginButton");
			}
		...
		Dialog.prototype.setEndButton = function (oButton) {
			...
			if (oButton && oButton.isA("sap.m.Button") && sTheme.startsWith("sap_fiori_")) {
				...
				oButton.addStyleClass("eyDDialogEndButton");
			}
		...
		Dialog.prototype.getAggregation = function (sAggregationName, oDefaultForCreation, bPassBy) {
			...
			if (sAggregationName === 'buttons' && originalResponse.length === 0) {
				var btn=this.getBeginButton() && originalResponse.push(this.getBeginButton());
				var btn2=this.getEndButton() && originalResponse.push(this.getEndButton());
			}
			...
			if (sIcon) {
				if (sIcon !== this.getIcon()) {
					if (this._iconImage) {
						...
						}, sap.m.Image).addStyleClass("eyDDialogIcon");
		...
		Dialog.prototype.setVerticalScrolling = function (bValue) {
			...
			this.$().toggleClass("eyDDialogVerScrollDisabled", !bValue);
		...
		Dialog.prototype.setHorizontalScrolling = function (bValue) {
			...
			this.$().toggleClass("eyDDialogHorScrollDisabled", !bValue);
		...
		function isHeaderClicked(eventTarget) {
			...
			return $target.hasClass('eyDDialogTitle');
		}
		...
		if (Device.system.desktop) {
			...
			Dialog.prototype.ondblclick = function (e) {
				if (isHeaderClicked(e.target)) {
					...
					//call the reposition
				var assignDialogVar=this.oPopup && this.oPopup._applyPosition(this.oPopup._oLastPosition, true);
					this._$dialog.removeClass('eyDDialogTouched');
			...
			Dialog.prototype.onmousedown = function (e) {
				...
				var bResize = $target.hasClass('eyDDialogResizeHandler') && this.getResizable();
				...
				function mouseUpHandler() {
					...
					if (bResize) {
						that._$dialog.removeClass('eyDDialogResizing');
				...
				if ((isHeaderClicked(e.target) && this.getDraggable()) || bResize) {
					...
					that._$dialog.addClass('eyDDialogDisableTransition');
					//remove the transform translate
					that._$dialog.addClass('eyDDialogTouched');
				...
				if (isHeaderClicked(e.target) && this.getDraggable()) {
					...
					that._$dialog.addClass('eyDDialogResizing');
